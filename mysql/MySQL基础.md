- [第一章 了解SQL](#第一章-了解sql)
	- [1.数据库基础](#1数据库基础)
		...
		- [5.主键](#5主键)
			- [主键的好习惯:](#主键的好习惯)
	- [2.什么是SQL](#2什么是sql)
- [第二章 MySQL简介](#第二章-mysql简介)
	- [1 什么是MySQL](#1-什么是mysql)
		- [1.客户机-服务器软件](#1客户机-服务器软件)
		- [2.MySQL版本](#2mysql版本)
	- [2.MySQL工具](#2mysql工具)
		- [1.MySQL命令行实用程序](#1mysql命令行实用程序)
		- [2.MySQL Administrator](#2mysql-administrator)
		- [3.MySQL Query Browser](#3mysql-query-browser)
- [第三章 使用MySQL](#第三章-使用mysql)
	- [1.连接](#1连接)
	- [2.选择数据库](#2选择数据库)
	- [3.了解数据库和表](#3了解数据库和表)
- [第四章 检索数据](#第四章-检索数据)
	- [1.select语句](#1select语句)
	- [2.检索单个列](#2检索单个列)
	- [3.检索多个列](#3检索多个列)
		- [数据表示](#数据表示)
	- [4.检索所有列](#4检索所有列)
	- [5.检索不同的行](#5检索不同的行)
	- [6.限制结果](#6限制结果)
	- [7.使用完全限定的表名](#7使用完全限定的表名)
- [第五章 排序检索数据](#第五章-排序检索数据)
	- [1.排列数据](#1排列数据)
		- [子句](#子句)
	- [2.按多个列排序](#2按多个列排序)
- [第六章 过滤数据](#第六章-过滤数据)
	- [1.使用where子句](#1使用where子句)
	- [2.where子句操作符](#2where子句操作符)
		- [1.检查单个值](#1检查单个值)
		- [2.不匹配检查](#2不匹配检查)
		- [3.范围值检查](#3范围值检查)
		- [4.空值检查](#4空值检查)
- [第七章 数据过滤](#第七章-数据过滤)
	- [1.组合where子句](#1组合where子句)
		- [1.and操作符](#1and操作符)
		- [2.or操作符](#2or操作符)
		- [3.计算次序](#3计算次序)
	- [2.in操作符](#2in操作符)
	- [3.not操作符](#3not操作符)
- [第八章 使用通配符过滤](#第八章-使用通配符过滤)
	- [1.like操作符](#1like操作符)
		- [1.百分号(%)通配符](#1百分号通配符)
		- [2.下划线(_)通配符](#2下划线_通配符)
	- [2.通配符使用技巧](#2通配符使用技巧)
- [第九章 用正则表达式进行搜索](#第九章-用正则表达式进行搜索)
	- [1.正则表达式介绍](#1正则表达式介绍)
	- [2.使用MySQL正则表达式](#2使用mysql正则表达式)
		- [1.基本字符匹配](#1基本字符匹配)
			- [regexp和like的区别](#regexp和like的区别)
		- [2.进行or匹配](#2进行or匹配)
		- [3.匹配几个字符之一](#3匹配几个字符之一)
		- [4.匹配范围](#4匹配范围)
		- [5.匹配特殊字符](#5匹配特殊字符)
		- [6.匹配字符类](#6匹配字符类)
		- [7.匹配多个实例](#7匹配多个实例)
		- [8.定位符](#8定位符)
- [第十章 创建计算字段](#第十章-创建计算字段)
	- [1.计算字段](#1计算字段)
	- [2.拼接字段](#2拼接字段)
		- [Trim()函数](#trim函数)
		- [使用别名](#使用别名)
...
- [第十一章 处理函数](#第十一章-处理函数)
	...
	- [2.使用函数](#2使用函数)
		- [1.用于处理文本串](#1用于处理文本串)
			- [常用函数](#常用函数)
		- [2.用于在数值上数据上进行算术操作](#2用于在数值上数据上进行算术操作)
		- [3.用于处理日期和时间值并从这些值中提取特定成分](#3用于处理日期和时间值并从这些值中提取特定成分)
			- [常用函数](#常用函数-1)
		- [4.返回DBMS正使用的特殊信息](#4返回dbms正使用的特殊信息)
- [第十二章 汇总数据](#第十二章-汇总数据)
	- [1.聚集函数](#1聚集函数)
		- [常用函数](#常用函数-2)
	- [2.聚集不同值](#2聚集不同值)
...
- [第十三章 分组数据](#第十三章-分组数据)
	...
	- [4.分组和排序](#4分组和排序)
		- [order by与group by的区别](#order-by与group-by的区别)
	- [5.select子句顺序](#5select子句顺序)
- [第十四章 使用子查询](#第十四章-使用子查询)
	- [1.子查询](#1子查询)
	- [2.作为计算字段使用子查询](#2作为计算字段使用子查询)
		- [相关子查询](#相关子查询)
- [第十五章 联结表](#第十五章-联结表)
	- [1.联结](#1联结)
		- [1.关系表](#1关系表)
		- [2.为什么使用外联结](#2为什么使用外联结)
	- [2.创建联结](#2创建联结)
		- [1.where子句的重要性](#1where子句的重要性)
		- [2.内部联结](#2内部联结)
	- [3.联结多个表](#3联结多个表)
- [第十六章 创建高级联结](#第十六章-创建高级联结)
	- [1.使用表别名](#1使用表别名)
	- [2.使用不同类型的联结](#2使用不同类型的联结)
		- [1.自联结](#1自联结)
		- [2.自然联结](#2自然联结)
		- [3.外部联结](#3外部联结)
			- [左联结left joijn](#左联结left-joijn)
		- [右联结right join](#右联结right-join)
	- [3.使用带聚集函数的联结](#3使用带聚集函数的联结)
	- [4.使用联结和联结条件](#4使用联结和联结条件)
- [第十七章 组合查询](#第十七章-组合查询)
	- [1.组合查询](#1组合查询)
	- [2.创建组合查询](#2创建组合查询)
...
- [第十九章 插入数据](#第十九章-插入数据)
	- [1.数据插入](#1数据插入)
- [第二十章 更新和删除数据](#第二十章-更新和删除数据)
	- [更新](#更新)
	- [删除](#删除)
- [第二十一章 创建和操纵表](#第二十一章-创建和操纵表)
	- [创建表](#创建表)
	- [更新表](#更新表)
	- [删除表](#删除表)
	- [重命名表](#重命名表)
- [第二十二章 使用视图](#第二十二章-使用视图)
	- [视图](#视图)
- [第二十三章 存储过程](#第二十三章-存储过程)
- [第二十四章  游标](#第二十四章--游标)
- [第二十五章 触发器](#第二十五章-触发器)
- [第二十六章 管理事务处理](#第二十六章-管理事务处理)
	- [几个常见概念](#几个常见概念)
		- [transcation:一组SQL语句](#transcation一组sql语句)
		- [rollback:撤销这一组SQL的提交](#rollback撤销这一组sql的提交)
		- [commit:将未存储的语句执行结果写入数据库表中](#commit将未存储的语句执行结果写入数据库表中)
		- [savepoint:一个事务中的临时占位符,可以针对其发布rollback](#savepoint一个事务中的临时占位符可以针对其发布rollback)
# 第一章 了解SQL
