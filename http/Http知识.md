参考资料---《图解HTTP协议》
- [一、了解Web及网络基础](#一了解web及网络基础)
  - [使用HttP协议访问web](#使用http协议访问web)
  - [HTTP的诞生](#http的诞生)
  - [网络基础TCP/IP-重点](#网络基础tcpip-重点)
    - [TCP/IP协议族](#tcpip协议族)
    - [TCP/IP的分层管理](#tcpip的分层管理)
      - [应用层](#应用层)
      - [传输层](#传输层)
      - [网络层](#网络层)
      - [链路层](#链路层)
    - [TCP/IP的通信传输流](#tcpip的通信传输流)
  - [与HTTP关系密切的协议：IP、TCP&DNS](#与http关系密切的协议iptcpdns)
    - [负责传输的IP协议](#负责传输的ip协议)
    - [负责确保可靠性的TCP协议](#负责确保可靠性的tcp协议)
      - [三次握手策略](#三次握手策略)
  - [负责域名解析的DNS服务](#负责域名解析的dns服务)
  - [各种协议与HTTP协议的关系](#各种协议与http协议的关系)
  - [URI和URL](#uri和url)
      - [绝对URI](#绝对uri)
      - [绝对URL](#绝对url)
      - [相对URL](#相对url)
- [二、简单的HTTP协议](#二简单的http协议)
  - [HTTP协议用于客户端和服务端之间的通信](#http协议用于客户端和服务端之间的通信)
  ...
  - [持久连接节省通信量](#持久连接节省通信量)
      - [持久连接](#持久连接)
      - [管线化](#管线化)
  - [使用Cookie的状态管理](#使用cookie的状态管理)
- [三、HTTP内的HTTP信息](#三http内的http信息)
  - [HTTP报文](#http报文)
  - [请求报文和响应报文的结构](#请求报文和响应报文的结构)
  - [编码提升传输速率-重点](#编码提升传输速率-重点)
    - [报文主体和实体主体的差异](#报文主体和实体主体的差异)
    - [压缩传输的内容编码](#压缩传输的内容编码)
    - [分割发送的分块传输编码](#分割发送的分块传输编码)
    - [发送多种数据的多部分集合](#发送多种数据的多部分集合)
    - [获取部分内容的范围请求](#获取部分内容的范围请求)
    - [内容协商](#内容协商)
- [四、返回结果的HTTP状态码](#四返回结果的http状态码)
  - [2xx成功](#2xx成功)
  - [3xx重定向](#3xx重定向)
  - [4xx 客户端错误](#4xx-客户端错误)
  - [5xx 服务端错误](#5xx-服务端错误)
- [五、与HTTP协作的web服务器](#五与http协作的web服务器)
  - [单台虚拟主体实现了多个域名](#单台虚拟主体实现了多个域名)
  - [通信数据转发程序：代理、网关和隧道](#通信数据转发程序代理网关和隧道)
    - [网关](#网关)
    - [代理](#代理)
      - [缓存代理-Cache Proxy](#缓存代理-cache-proxy)
...
- [六、报文首部](#六报文首部)
  - [HTTP报文首部](#http报文首部)
  - [HTTP首部字段](#http首部字段)
    - [首部字段传递重要信息](#首部字段传递重要信息)
    - [HTTP首部字段结构](#http首部字段结构)
    - [4种HTTP首部字段类型](#4种http首部字段类型)
      - [通用首部字段](#通用首部字段)
      - [请求首部字段-重点](#请求首部字段-重点)
      - [响应首部字段-重点](#响应首部字段-重点)
      - [实体首部字段](#实体首部字段)
